...
// Announcement banner: slides out to the left and the next one slides in
// from the right (generated by generate_cms_sections.py from
// content/announcement.json) instead of the old continuously-scrolling
// ticker. No prev/next controls - it only ever advances on its own. A
// single real message is duplicated by the generator into two identical
// slides so this still visibly cycles instead of sitting static.
//
// Resting (non-active, non-leaving) slides sit off-screen to the right,
// ready to be the next arrival - see .promo-banner-slide's default
// transform in css/style.css. Advancing adds .is-leaving to the outgoing
// slide (animates it off to the left) and .is-active to the incoming one
// (animates it in from the right); .is-leaving is removed once that
// transition ends so the slide resets to its off-screen-right resting
// spot, ready for its next turn.
(function(){
...
// ---- Mobile bottom CTA dock (.mcta-dock) ----
// Two presses to act: the first arms a button (shows its tooltip label,
// slides the gold bubble under it); the second press on that same,
// already-armed button navigates. A finger dragged across the dock
// previews each button it passes over the same way a first tap would -
// it only counts as a confirming press if the gesture starts and ends
// on the button that was already armed, so lifting mid-drag never fires
// a navigation by accident.
(function(){
  document.querySelectorAll('.mcta-dock').forEach(initMctaDock);

  function initMctaDock(dock){
    const buttons = Array.from(dock.querySelectorAll('.mcta-btn'));
    const bubble = dock.querySelector('.mcta-bubble');
    ...
    let pointerId = null;
    let startBtn = null;
    let startWasArmed = false;
    let currentBtn = null;

    function moveBubbleTo(btn){
      const dockRect = dock.getBoundingClientRect();
      const btnRect = btn.getBoundingClientRect();
      bubble.style.width = btnRect.width + 'px';
      bubble.style.height = btnRect.height + 'px';
      bubble.style.transform = 'translate(' + (btnRect.left - dockRect.left) + 'px,' + (btnRect.top - dockRect.top) + 'px)';
    }

    function arm(btn){
      if(armedBtn && armedBtn !== btn) armedBtn.classList.remove('is-armed');
      armedBtn = btn;
      btn.classList.add('is-armed');
      dock.classList.add('has-armed');
      moveBubbleTo(btn);
      clearTimeout(disarmTimer);
      disarmTimer = setTimeout(disarmAll, 3000);
    }
    function disarmAll(){
      clearTimeout(disarmTimer);
      if(armedBtn) armedBtn.classList.remove('is-armed');
      armedBtn = null;
      dock.classList.remove('has-armed');
    }
    ...
    dock.addEventListener('pointerdown', function(e){
      const btn = btnAtPoint(e.clientX, e.clientY);
      if(!btn) return;
      pointerId = e.pointerId;
      startBtn = btn;
      startWasArmed = armedBtn === btn;
      currentBtn = btn;
      arm(btn);
    });
    ...
    function finishPress(e){
      if(e.pointerId !== pointerId) return;
      pointerId = null;
      if(currentBtn && currentBtn === startBtn && startWasArmed){
        activate(currentBtn);
      }
      startBtn = null;
      currentBtn = null;
    }
    dock.addEventListener('pointerup', finishPress);
    dock.addEventListener('pointercancel', function(e){
      if(e.pointerId !== pointerId) return;
      pointerId = null;
      startBtn = null;
      currentBtn = null;
    ...
    window.addEventListener('resize', function(){
      if(armedBtn) moveBubbleTo(armedBtn);
    });
  }
})();
